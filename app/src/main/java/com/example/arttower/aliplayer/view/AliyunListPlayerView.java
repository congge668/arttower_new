package com.example.arttower.aliplayer.view;
...
import com.example.arttower.aliplayer.adapter.AliyunRecyclerViewAdapter;
import com.example.arttower.aliplayer.adapter.PagerLayoutManager;
import com.example.arttower.aliplayer.listener.OnViewPagerListener;
import com.example.arttower.aliplayer.refresh.AlivcSwipeRefreshLayout;
...
public class AliyunListPlayerView extends FrameLayout {
     ...
     * 添加视频
     */
    public void addUrl(String videoUrl, String randomUUID) {
        if (mAliListPlayer != null) {
    ...
    public void setData(List<HomeBean.DataBean> dataBeans) {
        ...
        if (mRecyclerViewAdapter != null) {
            mRecyclerViewAdapter.setData(dataBeans);
     ...
     * 加载更多数据
     */
    public void addMoreData(List<HomeBean.DataBean> dataBeans) {
        if (dataBeans == null || dataBeans.size() < 10) {
            isEnd = true;
            return;
        } else {
            isEnd = false;
        ...
        if (mRecyclerViewAdapter != null) {
            mRecyclerViewAdapter.addMoreData(dataBeans);
        }
        hideRefresh();
    }
    ...
    private void clearNotShowVideo(List<HomeBean.DataBean> list) {
        Iterator<HomeBean.DataBean> it = list.iterator();
//        while (it.hasNext()) {
